...
```"""
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Always set answer_found to false if the answer to the question was not found in the information provided."
            },
            {
                "role": "user",
                "content": prompt,
        ...
        function_call={
            "name": "give_response",
            "arguments": json.dumps({"answer_found": False, "response": ""})
        }
    )
